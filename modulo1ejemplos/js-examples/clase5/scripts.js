...
function calcularIMC() {
    ...
    const valorIMC = imc(peso, altura).toFixed(2);
    let categoria = '';
    ...
    resultadoDiv.innerHTML = `Tu IMC es <strong>${valorIMC}</strong> (${categoria}).`;
}
...
function obtenerPokemon() {
    ...
    const errorDiv = document.getElementById('pokemonError');
...
async function serchCharacter() {
    ...
    try {
        ...
        // Validación por consola del objeto completo e ID
        console.log("Objeto recibido:", character);
        console.log("ID recibido de la API:", character.id);
        ...
        document.getElementById("characterResult").innerHTML = characterHTML;
    } catch (error) {
        console.error("Error:", error.message);
        document.getElementById("characterResult").innerHTML = `<p>Error: ${error.message}</p>`;
    }
}

//Pendiente buscador de capitulos de acuerdo al personaje previamente buscado